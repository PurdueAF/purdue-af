...
ax.fill_between([df["reg_date"].min(), df["reg_date"].max()], 
                [0, 0], [df["cumulative_users"].max() * 1.1, df["cumulative_users"].max() * 1.1],
                alpha=0.02, color='#007ACC')

# Plot the line with modern styling
line = ax.plot(df["reg_date"], df["cumulative_users"], 
               color='#007ACC', linewidth=3, alpha=0.9, 
               solid_capstyle='round')

# Add subtle markers at data points
ax.scatter(df["reg_date"], df["cumulative_users"], 
          color='#007ACC', s=20, alpha=0.7, zorder=5)

# Add a subtle area fill under the line
ax.fill_between(df["reg_date"], df["cumulative_users"], 
                alpha=0.1, color='#007ACC')
...
ax.set_xlim(df["reg_date"].min() - pd.Timedelta(days=5), 
...
# Add a subtle title
ax.text(0.02, 0.98, 'Purdue Analysis Facility Users', 
...
ax.text(0.98, 0.5, f'{current_users}', 
        transform=ax.transAxes, fontsize=24, fontweight=700,
        color='#007ACC', horizontalalignment='right',
        verticalalignment='center')

# Add "users" label
ax.text(0.98, 0.35, 'users', 
        transform=ax.transAxes, fontsize=12, fontweight=400,
        color='#666666', horizontalalignment='right',
        verticalalignment='center')
