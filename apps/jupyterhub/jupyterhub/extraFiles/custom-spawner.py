    ...
    async def authenticate(self, handler, data=None):
        ...
        if domain == "purdue.edu":
            ...
            with open("/etc/secrets/af-auth-purdue/userlist") as file:
                if username not in {line.strip() for line in file}:
                    raise web.HTTPError(
                        403,
...
async def pre_spawn_start(spawner):
    auth_state = await spawner.user.get_auth_state()
    if auth_state and "name" in auth_state:
        spawner.environment["USERNAME"] = auth_state["name"]
...
c.KubeSpawner.pre_spawn_start = pre_spawn_start
