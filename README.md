...
[Flux](https://github.com/fluxcd/flux2) from two channels: **production** is
...
| --- | --- | --- | --- |
| Sessions | [`jupyterhub`](apps/jupyterhub/jupyterhub) | [JupyterHub](https://github.com/jupyterhub/zero-to-jupyterhub-k8s) — [JupyterLab](https://github.com/jupyterlab/jupyterlab) and [code-server](https://github.com/coder/code-server) profiles, [CILogon](https://www.cilogon.org) auth (Purdue / CERN / FNAL identities) | ![production][production-jupyterhub-jupyterhub] |
|  | [`purdue-af`](docker/purdue-af) | the session image: [nvidia/cuda:12.4.1-devel-rockylinux8](https://hub.docker.com/r/nvidia/cuda) base, [pixi](https://github.com/prefix-dev/pixi) `base` environment, OSG grid clients ([XRootD](https://github.com/xrootd/xrootd), gfal2) | ![production][production-docker-purdue-af] |
|  | [`jupyterhub-ssh`](apps/jupyterhub/jupyterhub-ssh) | SSH and SFTP access into a running session | ![production][production-jupyterhub-jupyterhub-ssh] |
|  | [`userlist-sync`](apps/jupyterhub/userlist-sync) | syncs the authorized Purdue and CERN user lists into the hub's secrets | ![production][production-jupyterhub-userlist-sync] |
|  | [`af-x509-secrets`](apps/jupyterhub/af-x509-secrets) | keeps the grid X.509 proxy secret current for ServiceX | ![production][production-jupyterhub-af-x509-secrets] |
|  | [`database-backup`](apps/jupyterhub/database-backup) | twice-daily backup of the hub database, 5 kept | ![production][production-jupyterhub-database-backup] |
...
| Storage | [`infrastructure`](apps/infrastructure) | the PVCs and pull secrets every session depends on: the [CVMFS](https://github.com/cvmfs/cvmfs) mount, the shared `/work` volume, registry credentials | ![production][production-infrastructure] ![experimental][experimental-infrastructure] |
...
| Observability | [`prometheus`](apps/monitoring/prometheus) | [Prometheus](https://github.com/prometheus/prometheus) — metrics and alerting rules | ![production][production-monitoring-prometheus] |
|  | [`grafana`](apps/monitoring/grafana) | [Grafana](https://github.com/grafana/grafana) — dashboards, provisioned from this repo | ![production][production-monitoring-grafana] |
|  | [`af-monitoring`](apps/monitoring/af-monitoring) | two purpose-built exporters: per-session metrics and per-node health | ![production][production-monitoring-af-monitoring] ![experimental][experimental-monitoring-af-monitoring] |
...
|  | [`af-users-graph`](apps/af-utils/af-users-graph) | cumulative registered-user plot, from the hub database backup | ![production][production-af-utils-af-users-graph] |
...
[production-jupyterhub-jupyterhub]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-jupyterhub-jupyterhub.json
[production-docker-purdue-af]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-docker-purdue-af.json
[production-jupyterhub-jupyterhub-ssh]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-jupyterhub-jupyterhub-ssh.json
[production-jupyterhub-userlist-sync]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-jupyterhub-userlist-sync.json
[production-jupyterhub-af-x509-secrets]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-jupyterhub-af-x509-secrets.json
[production-jupyterhub-database-backup]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-jupyterhub-database-backup.json
...
[production-infrastructure]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-infrastructure.json
...
[production-monitoring-prometheus]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-monitoring-prometheus.json
[production-monitoring-grafana]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-monitoring-grafana.json
[production-monitoring-af-monitoring]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-monitoring-af-monitoring.json
...
[production-af-utils-af-users-graph]: https://img.shields.io/endpoint?url=https://raw.githubusercontent.com/PurdueAF/purdue-af/status/badges/production-af-utils-af-users-graph.json
