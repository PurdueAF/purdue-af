...
directories = {
    "home": glob.glob("/home/*")[0],
    "work": f"/work/users/{username}/"
}

metrics = {}
for dl in directories.keys():
    metrics[f"{dl}_dir_used"] = Gauge(f'af_{dl}_dir_used_kb', f'Used storage in {dl} directory mounted to an Analysis Facility pod')
    metrics[f"{dl}_dir_size"] = Gauge(f'af_{dl}_dir_size_kb', f'Total storage in {dl} directory mounted to an Analysis Facility pod')
    metrics[f"{dl}_dir_util"] = Gauge(f'af_{dl}_dir_util', f'Storage utilization in {dl} directory mounted to an Analysis Facility pod')
    metrics[f"{dl}_dir_last_accessed"] = Gauge(f'af_{dl}_dir_last_accessed', f'Last accessed timestamp for {dl} directory in Analysis Facility')
