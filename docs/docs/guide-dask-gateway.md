...
| **Cons** | • Unavailable to CERN/FNAL users<br>• Scheduling workers can be slow due to competition with CMS production jobs | • Limited total amount of resources<br>• Retreiving detailed worker info can be non-trivial for users (but easy for admins) |
...
2. Via the general Purdue AF monitoring page, in the "Slurm metrics" and "Dask metrics" sections
   of the [monitoring dashboard](https://cms.geddes.rcac.purdue.edu/grafana/d/purdue-af-dashboard/purdue-analysis-facility-dashboard){ target="_blank" }.
