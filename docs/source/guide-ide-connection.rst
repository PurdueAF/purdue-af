IDE connection guide (VSCode, Cursor, Antigravity)
==================================================
...
2. Set up SSH keys on local machine
-----------------------------------------
...
3. Configure SSH on local machine
----------------------------------
...
5. Create a JupyterHub token
-----------------------------

#. In the JupyterLab menu, go to ``File -> Hub Control Panel``.
#. Click the ``Token`` tab.
#. Click ``Request new API token``.
#. Copy the token string and use it to replace ``TOKEN`` in the ``~/.ssh/config`` file.


6. Configure SSH access on the Purdue AF side
---------------------------------------------------
